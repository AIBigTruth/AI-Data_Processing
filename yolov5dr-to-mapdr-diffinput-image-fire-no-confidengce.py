...
path ='F://github/yolov5-v5/runs/detect/detect-fire-smoke-100-0.5-test/labels/'
path_image ='F://github/yolov5-v5/runs/detect/detect-fire-smoke-100-0.5-test/image/'
...
for file in files:
    ...
    for i in range(x):
        for j in range(y):
            ...
            c = float(dataMat[i][0])
            if c == 0:
                newData1 = ['fire',  int(x1), int(y1), int(x2), int(y2)]

            else:
                newData1 = ['smoke',  int(x1), int(y1), int(x2), int(y2)]
